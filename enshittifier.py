...
def add_family_aliases(font: TTFont, aliases: list[str], verbose: bool = True):
    ...
    # patched shadow (~/Library/Fonts/Arial Narrow.ttf reporting
    # "ArialNarrow") no longer dedupe against the unpatched system font
    # (reporting "Arial Narrow"). We only fill it in when missing so we
    # don't clobber a font's intentionally distinct Typographic Family
    # (e.g. "Helvetica Neue Bold" / typographic "Helvetica Neue").
